...
class Optimizer_SGD:
    ...
    def update_params(self, layer) -> None:
        ...
        """
        if self.momentum:
            if not hasattr(layer, 'sgd_weight_momentums'):
                layer.sgd_weight_momentums = np.zeros_like(layer.weights)
                layer.sgd_bias_momentums = np.zeros_like(layer.biases)

            weight_updates = (
                self.momentum * layer.sgd_weight_momentums -
                self.current_learning_rate * layer.dweights
            )
            layer.sgd_weight_momentums = weight_updates

            bias_updates = (
                self.momentum * layer.sgd_bias_momentums -
                self.current_learning_rate * layer.dbiases
            )
            layer.sgd_bias_momentums = bias_updates
        else:
...
class Optimizer_Adagrad:
    ...
    def update_params(self, layer) -> None:
        ...
        Returns:
            None
        """
        if not hasattr(layer, 'adagrad_weight_cache'):
            layer.adagrad_weight_cache = np.zeros_like(layer.weights)
            layer.adagrad_bias_cache = np.zeros_like(layer.biases)

        layer.adagrad_weight_cache += layer.dweights**2
        layer.adagrad_bias_cache += layer.dbiases**2

        layer.weights += -self.current_learning_rate * layer.dweights / (
            np.sqrt(layer.adagrad_weight_cache) + self.epsilon
        )
        layer.biases += -self.current_learning_rate * layer.dbiases / (
            np.sqrt(layer.adagrad_bias_cache) + self.epsilon
        )
...
class Optimizer_RMSprop:
    ...
    def update_params(self, layer) -> None:
        ...
        Returns:
            None
        """
        if not hasattr(layer, 'rmsprop_weight_cache'):
            layer.rmsprop_weight_cache = np.zeros_like(layer.weights)
            layer.rmsprop_bias_cache = np.zeros_like(layer.biases)

        layer.rmsprop_weight_cache = (
            self.rho * layer.rmsprop_weight_cache +
            (1 - self.rho) * layer.dweights**2
        )

        layer.rmsprop_bias_cache = (
            self.rho * layer.rmsprop_bias_cache +
            (1 - self.rho) * layer.dbiases**2
        )

        layer.weights += -self.current_learning_rate * layer.dweights / (
            np.sqrt(layer.rmsprop_weight_cache) + self.epsilon
        )
        layer.biases += -self.current_learning_rate * layer.dbiases / (
            np.sqrt(layer.rmsprop_bias_cache) + self.epsilon
        )
...
class Optimizer_Adam:
    ...
    def update_params(self, layer) -> None:
        ...
        Returns:
            None
        """
        if not hasattr(layer, 'adam_weight_cache'):
            layer.adam_weight_momentums = np.zeros_like(layer.weights)
            layer.adam_weight_cache = np.zeros_like(layer.weights)
            layer.adam_bias_momentums = np.zeros_like(layer.biases)
            layer.adam_bias_cache = np.zeros_like(layer.biases)

        layer.adam_weight_momentums = (
            self.beta_1 * layer.adam_weight_momentums + (1 - self.beta_1) *
            layer.dweights
        )

        layer.adam_bias_momentums = (
            self.beta_1 * layer.adam_bias_momentums + (1 - self.beta_1) *
            layer.dbiases
        )
        ...
        layer.adam_weight_cache = (
            self.beta_2 * layer.adam_weight_cache + (1 - self.beta_2) *
            layer.dweights ** 2
        )

        layer.adam_bias_cache = (
            self.beta_2 * layer.adam_bias_cache + (1 - self.beta_2) *
            layer.dbiases ** 2
        )
...
class Accuracy:
    ...
    def calculate(self, predictions, y):
        ...
        self.accumulated_count += comparisons.size
